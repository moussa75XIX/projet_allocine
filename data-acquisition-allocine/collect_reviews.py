...
def main():
    urls_dicts = json.load(open(os.path.join(PATH_URLS_REVIEWS_TO_COLLECT, COLLECT_DATE + '_reviews_urls_to_collect.json'), 'r'))
    ...
    for url_dict in urls_dicts:
        ...
        if url_dict['collected'] == 'no':
            ...
            try:
                ...
                reviews_data = collect_reviews_data(driver, url_dict['url'])

                with open(os.path.join(PATH_REVIEWS, COLLECT_DATE + '_data_reviews_' + str(time.strftime("%Y_%m_%d_%H_%M_%S")) + '.json'), 'w', encoding='utf-8') as file_to_dump:
                    json.dump(reviews_data, file_to_dump, indent=2, ensure_ascii=False)

                # Change the status of the current url and save it.
                # The url is set as 'yes'.
                url_dict['collected'] = 'yes'
                print("[LOG] All the reviews data have been collected for the current url")

                with open(os.path.join(PATH_URLS_REVIEWS_TO_COLLECT, COLLECT_DATE + '_reviews_urls_to_collect.json'),'w') as file_to_dump:
